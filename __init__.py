...
class InspectInBackground(BackgroundTaskThread):
    def __init__(self, bv: BinaryView):
        BackgroundTaskThread.__init__(self, "Annotating functions...", True)
        self.bv = bv
    ...
    def comment_at(self, addr: int, comment: str):
        ...
        original_comment = "\n".join([
            line for line in original_comment.split("\n") if "REF: " not in line
        ])
    ...
    def run(self):
        ...
        for addr, var in self.bv.data_vars.items():
            if isinstance(var.type, Type):
                if (
                    var.type.type_class == TypeClass.ArrayTypeClass
                    and "char" in var.type.get_string()
                ):
                    # is string
                    for ref in var.code_refs:
                        if isinstance(ref.function, Function):
                            ...
                            try:
                                functions[ref.function.start].append(
                                    str(var.value[:-1], "utf-8").replace("\n", "\\n")
                                )
                            except Exception as e:
                                print(
                                    f'failed to set string reference to {var.address:04X} from {ref.function.start:04X}: {e} "{var.value}"'
                                )
...
def inspect(bv: BinaryView):
    if bv.analysis_info.state != 2:
        print(
            "Binja analysis still ongoing, please run this plugin only after analysis completes."
        )
    else:
...
PluginCommand.register(
    "Annotate references to functions with strings used",
